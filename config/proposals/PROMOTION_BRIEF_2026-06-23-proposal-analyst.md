# Promotion Brief — 2026-06-23 (Backtest Analyst, spec 06 §4)
...
No state was mutated by this run: ledger and HEAD MD5s identical before/after, zero trials
consumed, zero config-store changes.
...
(or change no lever) and can never clear the deterministic validation gate against current HEAD v4.

I ran `scripts/process_proposal.py` on each. The gate rejected all five at **validation**,
*before any backtest* — so no in-sample/OOS/lockbox numbers are produced, no trial is recorded,
and the config store is never touched:
...
every verdict. Sandbox note: `pyarrow` was pip-installed to let the backtester read the parquet;
all five still stopped at validation regardless.*
