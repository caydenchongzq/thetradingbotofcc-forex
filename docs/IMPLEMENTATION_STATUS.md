...
`166 passed`. Run: `python -m pytest`.
...
- `scripts/process_proposal.py` — run an improvement-loop proposal through the real backtester + gates, record the trial, `--approve` to promote (**A6**).
...
| **07 ops** | Watchdog, alerting, supervision, backups — not started. |
