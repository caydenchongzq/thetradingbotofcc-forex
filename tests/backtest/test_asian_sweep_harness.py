"""AsianSweepFade through the REAL harness (spec 08 candidate, dev-only)."""
...
from tests.engine.test_asian_sweep import CFG, SWEEP_SHORT, with_window_bars
...
def _run(sweep: bool):
    ...
    last = bt[-1]
    px = last.close
    for k in range(1, 8):                      # drift DOWN so the short can resolve at TP
    ...
    engine = EventDrivenBacktester(
        AsianSweepFade(CFG), RiskGovernor(RiskConfig()), SM,
        CostModel(pip_size=0.0001, pip_value_per_lot_usd=10.0),
        initial_balance=100_000.0)
    req = BacktestRequest(strategy_name="AsianSweepFade", config_version=0,
